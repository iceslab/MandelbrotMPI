...
W pierwszej kolejności wykonywana jest kompilacja projektu. W pliku źródłowym \textit{Master.cpp} jest makrodefinicja \textit{\_LOCALHOST\_ONLY\_}, dzięki której można wyspecyfikować czy program ma łączyć się ze zdalną bazą danych, której adres zmodyfikować można w pliku \textit{NetCpp/comm.h} czy opis sceny ma być pobierany z kodu źródłowego. Następnie pliki wykonywalne rozsyłane są za pośrednictwem programu rsync do poszczególnych node'ów. Polecenie to wygląda następująco:\\
